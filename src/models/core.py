...
from allennlp.data.token_indexers import WordpieceIndexer, wordpiece_indexer
...
from transformers import RobertaTokenizer, AutoModel, XLMTokenizer
...
class GenericWordpieceIndexer(TokenIndexer):
    """
    ...
    def __init__(
        ...
        if never_lowercase is None:
            # Use the defaults
            self._never_lowercase = set(wordpiece_indexer._NEVER_LOWERCASE)
        else:
    ...
    def tokens_to_indices(
        ...
        token_wordpiece_ids = [
            [self.vocab.get(wordpiece, self.unk_token) for wordpiece in self.wordpiece_tokenizer(token)]
            for token in text
        ]
    ...
    def as_padded_tensor(
        ...
        return {
            key: torch.LongTensor(pad_sequence_to_length(val, desired_num_tokens[key], default_value=self.padding))
...
@TokenIndexer.register("xlm-pretrained")
class PretrainedXLMIndexer(GenericWordpieceIndexer):
    def __init__(
            self,
        ...
        xlm_tokenizer = XLMTokenizer.from_pretrained(pretrained_model, do_lowercase=do_lowercase)
        super().__init__(
            vocab=xlm_tokenizer.encoder,
            wordpiece_tokenizer=xlm_tokenizer.tokenize,
            pad_token=xlm_tokenizer.pad_token_id,
            unk_token=xlm_tokenizer.unk_token_id,
            namespace="xlm",
...
class PretrainedRoBERTaIndexer(GenericWordpieceIndexer):
    def __init__(
            self,
            pretrained_model: str,
            use_starting_offsets: bool = False,
        ...
        roberta_tokenizer = RobertaTokenizer.from_pretrained(pretrained_model, do_lowercase=do_lowercase)
        super().__init__(
            vocab=roberta_tokenizer.encoder,
            wordpiece_tokenizer=roberta_tokenizer.tokenize,
            pad_token=roberta_tokenizer.pad_token_id,
            unk_token=roberta_tokenizer.unk_token_id,
            namespace="roberta",
            use_starting_offsets=use_starting_offsets,
            max_pieces=max_pieces,
            do_lowercase=do_lowercase,
            never_lowercase=never_lowercase,
            start_tokens=[roberta_tokenizer.cls_token],
            end_tokens=[roberta_tokenizer.sep_token],
...
if __name__ == "__main__":
    token_indexer = PretrainedXLMIndexer(
        pretrained_model="xlm-mlm-en-2048",
        do_lowercase=False,
        truncate_long_sequences=False
    )
    indexers = {"tokens": token_indexer}
    reader, lemma2synsets, label_vocab, mfs_dictionary = AllenWSDDatasetReader.get_wnoffsets_dataset(indexers)
    training_paths = [
        "/home/tommaso/Documents/data/WSD_Evaluation_Framework_2.0/Training_Corpora/SemCor/semcor.data.xml"]
    train_ds = reader.read(training_paths)
    iterator = BasicIterator(maximum_samples_per_batch=("tokens_length", 1000),
                             cache_instances=True
                             )
    iterator.index_with(Vocabulary())
    generator = iterator(train_ds,
                         num_epochs=1,
                         shuffle=False)
    model = BertEmbedder(AutoModel.from_pretrained("xlm-mlm-en-2048", output_hidden_states=True), top_layer_only=True)
    # model = PretrainedBertEmbedder("bert-base-cased")
    for b in generator:
        print(model(b["tokens"]["tokens"]))
